...
void Foam::multiphaseSystem::solveAlphas()
{
    ...
    forAllIter(PtrDictionary<phaseModel>, phases_, iter)
    {
        ...
        forAllIter(PtrDictionary<phaseModel>, phases_, iter2)
        {
            ...
            phir += min(cAlphaFunc(phase,phase2)*phic,max(phic))*
                    nHatf(phase,phase2);
...
Foam::tmp<Foam::surfaceScalarField> Foam::multiphaseSystem::cAlphaFunc
(
    const volScalarField& alpha1,
    const volScalarField& alpha2
) const
{
    ...
    surfaceScalarField cAlphaGamma
    (
        mag(gradAlphaf)/max(mag(gradAlphaf))
    );
    
    const double gammaStar = 0.4;
    
    forAll(cAlphaGamma.internalField,i)
    {
        if (cAlphaGamma.internalField()[i] < gammaStar)
        {
            cAlphaGamma.internalField()[i] = 0.0;
        }
        else
        {
            cAlphaGamma.internalField()[i] = 1.0;
        }
    }

    forAll(cAlphaGamma.boundaryField,j)
    {
        if (cAlphaGamma.boundaryField()[j] < gammaStar)
        {
            cAlphaGamma.boundaryField()[j] = 0.0;
        }
        else
        {
            cAlphaGamma.boundaryField()[j] = 1.0;
        }
    }
